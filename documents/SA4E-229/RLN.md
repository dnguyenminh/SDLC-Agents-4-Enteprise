...
|---------|------|--------|---------|
| 1.0 | 2026-08-28 | DevOps Agent | Initiate document |
...
|-----------|--------|-------------|
| Backend MCP server (`sdlc-agent-4-enterprise-server`) | Rebuilt / republished | New tool compiled into `dist/`; image tag `:1.39.0` |
...
|---|-------|--------|------------|------------|
| 1 | VS Code/Kiro extension does not yet expose `jira_download_attachment` in-process (only the backend MCP server does) | Extension users must route through the backend MCP server | Use backend MCP server | Future ticket |
| 2 | Attachments > 10MB may approach the <5s NFR under load | Slower downloads | Stream / increase timeout | TBD |
| 3 | Live Jira integration (TC-700) + perf NFR (TC-600/601) deferred to live environment | Not validated in CI | Validate in SIT/UAT | Next release |
...
|------|--------|---------------|
| 1 | Build & test backend | 5 min |
| 2 | Publish release (npm / docker) | 5 min |
| 3 | Update & restart running service | 3 min |
| 4 | Verification (health + smoke) | 5 min |
| **Total** | | **~18 min** |
