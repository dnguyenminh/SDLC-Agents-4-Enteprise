...
If not available from tickets, to be confirmed with stakeholders.
...
**Step 1:** PiAgent Executor invokes a tool via Pi SDK and receives tool call metadata including tool_use_id in Pi format.

**Step 2:** Approval Adapter intercepts tool result and normalizes tool_use_id to extension format.

**Step 3:** Adapter forwards request to ToolApprovalGate with normalized identifiers and context.

**Step 4:** ToolApprovalGate evaluates policy and returns approval status, user input, or rejection.

**Step 5:** Adapter translates approval response back to Pi SDK compatible format.

**Step 6:** PiAgent Executor continues execution with approved tool result or requests clarification.

> **Note:** Adapter must be transparent to both Pi SDK and ToolApprovalGate; no changes to either core component.
...
- Approval context must include ticketKey, agentId, toolName.
...
|------|--------|------------|------------|
| Pi SDK tool_use_id format changes in future release | High | Medium | Abstract mapping behind interface, add unit tests |
| ToolApprovalGate APIs incompatible with Pi flow | High | Medium | Adapter pattern isolates changes; early spike |
| Missing audit trail during normalization | Medium | Low | Log all mappings with ticketKey and sessionId |

### 5.2 Assumptions

- ToolApprovalGate remains unchanged during migration.
- Pi SDK provides stable tool_use_id in current version.
...
|----------|-------------|---------|
| Performance | Adapter latency | < 10ms per tool call normalization |
| Security | Id mapping | No sensitive data in logs |
| Scalability | Concurrent sessions | Support 100+ parallel Pi sessions |
| Availability | | Adapter must not block approval flow |
...
| Term | Definition |
|------|------------|
| ToolApprovalGate | Existing human-in-the-loop approval component in LangGraph workflow |
| Pi SDK | @earendil-works/pi-agent-core SDK used for migration |
| tool_use_id | Identifier for a tool call, format differs between Pi SDK and extension |
...
| Epic SA4E-289 Description | Jira SA4E-289 |
