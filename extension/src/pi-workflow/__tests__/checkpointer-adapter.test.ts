...
import { CheckpointerAdapter, StateAdapter, PayloadTooLargeError, SerializationError } from "../checkpointer-adapter";
...
describe("StateAdapter", () => {
  ...
  it("should convert PiInternalState to PipelineState", () => {
    ...
    const pipeline = adapter.toPipelineState(piState);
  ...
  it("should convert PipelineState to PiInternalState", () => {
    ...
    const piState = adapter.fromPipelineState(pipeline);
...
describe("CheckpointerAdapter", () => {
  ...
  it("should throw on invalid UUID", async () => {
    ...
    await expect(adapter.putCheckpoint("invalid-uuid", piState)).rejects.toThrow("Invalid UUID");
  });
