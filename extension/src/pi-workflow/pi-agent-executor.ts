  ...
  async executeTurn(input: ExecuteTurnInput): Promise<PiAgentExecutionResult> {
    if (!input || !input.ticketKey || !TICKET_KEY_REGEX.test(input.ticketKey)) {
      return {
        ...
        toolCalls: [],
        streamChunks: [],
        error: { code: 'INVALID_INPUT', message: 'ticketKey is required and must be in format KEY-123' }
      };
    }
    ...
    if (!input.sessionId || input.sessionId.trim() === '') {
      return {
        ...
        toolCalls: [],
        streamChunks: [],
        error: { code: 'INVALID_INPUT', message: 'sessionId is required and cannot be empty' }
      };
    }
    ...
    if (!input.agentId || input.agentId.trim() === '') {
      return {
        ...
        toolCalls: [],
        streamChunks: [],
        error: { code: 'INVALID_INPUT', message: 'agentId is required and cannot be empty' }
      };
    }
    ...
    if (!input.messages || !Array.isArray(input.messages) || input.messages.length === 0) {
      return {
        ...
        toolCalls: [],
        streamChunks: [],
        error: { code: 'INVALID_INPUT', message: 'messages is required and must not be empty' }
      };
    }
    ...
    try {
      ...
      for await (const chunk of this.provider.stream({ prompt })) {
        ...
        if (chunk.type === 'text' && chunk.content) {
          ...
          const normalized = normalizeToolCall(chunk.toolCall as any);
      ...
      return {
        ...
        toolCalls,
        streamChunks,
        error: { code: err?.code || 'PI_EXECUTION_ERROR', message: err?.message || String(err) }
      };
    }
  }
}
