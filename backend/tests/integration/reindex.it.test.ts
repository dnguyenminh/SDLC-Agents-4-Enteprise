...
describe('SA4E-42 re-index integration', () => {
  ...
  it('IT-01 (repro): late connect makes tools discoverable within ≤5s', async () => {
    ...
    const row = await h.adapter.getAsync<{ schema_json: string }>('SELECT schema_json FROM mcp_tools WHERE name = ?', ['jira_create_issue']);
  ...
  it('IT-07: non-blocking read during in-flight refresh (BR-09)', async () => {
    ...
    expect(rows.some((r: any) => r.name === 'export_docx')).toBe(true); // current index visible
    ...
    expect(await names(h.adapter, 'atlassian')).toContain('jira_create_issue'); // new tools appear
    sub.stop();
  });
  ...
  it('IT-12: cross-server name collision is not silently hijacked (F-01)', async () => {
    ...
    await sub.settle('B');
    const row = await h.adapter.getAsync<{ server: string }>('SELECT server FROM mcp_tools WHERE name = ?', ['common_tool']);
