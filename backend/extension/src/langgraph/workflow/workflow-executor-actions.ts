    ...
    switch (action.type) {
      ...
      case "fetch_jira_recursive": {
        try {
          ...
          async function traverse(key: string, depth: number): Promise<void> {
            if (depth > 2 || visited.size >= 10) return;
            visited.add(key);
            const data = await node.callMcp("execute_dynamic_tool", { tool_name: "jira_get_issue", arguments: { issue_key: key, fields: "*all" } });
            allData += (depth === 0 ? "## MAIN: " : `## LINKED(depth=${depth}): `) + key + "\n\n" + data + "\n\n---\n\n";
            const linked = [...data.matchAll(/\b([A-Z][A-Z0-9]{1,9}-\d+)\b/g)].map(m => m[1]).filter(k => !visited.has(k));
            for (const lk of linked.slice(0, 4)) await traverse(lk, depth + 1);
          }
...
function resolvePath(path: string, ctx: ExecutionContext): string {
