 ...
 *
 * Autonomy Levels (Kiro spec):
 *   L1 — Strict: always pause for human approval, extra criteria added
 *   L2 — Normal: pause for human approval (default behavior)
 *   L3 — Auto: skip pause, auto-approve and proceed
 */

import { BaseNode } from "../core/base-node";
import { McpBridge } from "../core/mcp-bridge";
import { StreamHandler } from "../core/stream-handler";
import { PipelineState, QualityGateCheckpoint, QualityGateResult, SDLCPhase, AutonomyLevel } from "../core/state";
...
export class ApprovalNode extends BaseNode {
  ...
  async execute(state: PipelineState): Promise<Partial<PipelineState>> {
    const autonomyLevel: AutonomyLevel = state.autonomyLevel || "L2";
    ...
    if (autonomyLevel === "L3") {
      this.streamHandler.emitToken(
        this.nodeId,
        `[L3 Mode] Auto-approving quality gate for ${this.phase}`,
        state.currentStreamId
      );
      return {
        ...
        qualityGateResults: { [this.phase]: { passed: true, issues: [] } },
      };
    }

    const isL1 = autonomyLevel === "L1";
