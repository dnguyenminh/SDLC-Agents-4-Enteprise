 ...
 * SA4E-50: getKbEntries is now async — processKbEntries awaits it.
 */
...
export function insertAllNodes(
...
/** Collect KB document entries (async because getKbEntries is now async). */
async function processKbEntries(
  allEntries: Array<any>,
  sources: Record<string, number>,
  ksaGroupMap: Map<string, number>,
...
function readSymbolBatches(indexDb: Database.Database, allEntries: Array<any>, sources: Record<string, number>, ksaGroupMap: Map<string, number>, groupCounter: { value: number }, logger: Logger): void {
  ...
  while (true) {
    const rows = indexDb.prepare(
      `SELECT s.id, s.name, s.kind, f.path, f.language FROM symbols s LEFT JOIN files f ON f.id = s.file_id WHERE s.kind IN (${placeholders}) ORDER BY s.id ASC LIMIT ? OFFSET ?`,
...
function processCodeSymbols(allEntries: Array<any>, sources: Record<string, number>, ksaGroupMap: Map<string, number>, groupCounter: { value: number }, logger: Logger): void {
  ...
  try {
    ...
    if (hasTables && hasTables.cnt >= 2) readSymbolBatches(indexDb, allEntries, sources, ksaGroupMap, groupCounter, logger);
    indexDb.close();
  } catch (err: any) {
