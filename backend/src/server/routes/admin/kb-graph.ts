...
export function createKbGraphRoutes(ctx: AdminContext): Hono {
  ...
  app.get('/api/admin/kb/graph', (c) => {
    ...
    const result = getKbEntries(1, 500, 'created_at', 'desc', ctx.getRequestProjectId(c));
    let nodes: any[] = [];
    let edges: any[] = [];
    ...
    return c.json({ nodes, edges, stats: { totalNodes: nodes.length, totalEdges: edges.length, maxNodes, totalEntries: getKbEntryCount(ctx.getRequestProjectId(c)) } });
  });
  ...
  app.get('/api/admin/kb/graph/cluster/:clusterId', (c) => {
    ...
    const result = getKbEntries(1, 5000, 'created_at', 'desc', ctx.getRequestProjectId(c));
  ...
  app.post('/api/admin/kb/graph/sync', async (c) => {
    const user = ctx.requireAuth(c);
    if (user instanceof Response) return user;
    const permCheck = ctx.requirePermission(c, user.userId, 'GRAPH_VIEW');
