...
export function createKbTagsRoutes(ctx: AdminContext): Hono {
  ...
  app.put('/api/admin/kb/tags/:name', async (c) => {
    ...
    let renamed = 0;
    for (const [entryId, tags] of Object.entries(ctx.kbTags)) {
      const idx = tags.indexOf(oldName);
      if (idx !== -1) { tags[idx] = newName.trim(); renamed++; }
    }
    const dbRenamed = renameKbTag(oldName, newName.trim());
    recordAudit(user.userId, user.username, 'RENAME_TAG', 'kb', undefined, JSON.stringify({ oldName, newName: newName.trim(), entriesAffected: renamed + dbRenamed }));
  ...
  app.delete('/api/admin/kb/tags/:name', (c) => {
    ...
    recordAudit(user.userId, user.username, 'DELETE_TAG', 'kb', undefined, JSON.stringify({ tag: tagName, entriesAffected: removed + dbRemoved }));
  ...
  app.post('/api/admin/kb/tags/merge', async (c) => {
    ...
    recordAudit(user.userId, user.username, 'MERGE_TAGS', 'kb', undefined, JSON.stringify({ sourceTag, targetTag, entriesAffected: merged + dbMerged }));
  ...
  app.get('/api/admin/kb/tags/:name/entries', (c) => {
    ...
    const memEntries = memEntryIds.map(id => { const entry = getKbEntryById(id); if (!entry) return null; return { id: entry.id || entry.entry_id || id, source: entry.source || entry.title || 'Untitled', tier: entry.tier || entry.scope || 'SHARED', type: entry.content_type || entry.type || 'document', createdAt: entry.created_at || null }; }).filter(Boolean);
    const dbRows = getKbEntriesByTag(tagName, ctx.getRequestProjectId(c));
    ...
    if (Array.isArray(allowedTiers)) filtered = entries.filter((e: any) => allowedTiers.includes(e.tier));
